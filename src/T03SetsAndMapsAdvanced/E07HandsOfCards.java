...
public class E07HandsOfCards {
    public static void main(String[] args) {
        ...
        while (!input.equals("JOKER")) {
            ...
            String[] cardsArray = playerCards.split(", ");

            Set<String> cardsSet = new HashSet<>();
            cardsSet.addAll(Arrays.asList(cardsArray));

            if (!map.containsKey(playerName)) {
                map.put(playerName, cardsSet);
            } else {
                Set<String> currentCards = map.get(playerName);
                currentCards.addAll(cardsSet);
                map.put(playerName, currentCards);
            }

            input = scan.nextLine();
        }

        //име на играча -> списък с картите
    ...
    private static int getCardsPoints(Set<String> cards) {
        //намираме силата на всяка една карта и след това добавяме тази сила към сумата
        // символ -> стойност
